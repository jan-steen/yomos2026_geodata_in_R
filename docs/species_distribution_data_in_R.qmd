...
```{r}
gecko_traits <- read.csv("Madagaskar_Gecko_Traits.csv")
head(gecko_traits)
```
...
```{r}
shp_gecko <- read_sf("Madagaskar_geckos_GARD1.7.shp")
# vect("Madagaskar_geckos_GARD1.7.shp") is also an option
head(shp_gecko)
```
...
writeRaster(richness, "gecko_sp_richness_Madagascar.tif", overwrite=T) # save for later
plot(richness)
```
...
plot(richness_noc)
writeRaster(richness_noc, "richness_nocturnal_madagaskar.tif", overwrite=T)
...
plot(richness_diu)
writeRaster(richness_diu, "richness_diurnal_madagaskar.tif", overwrite=T)
```
...
overall_richness <- rast("gecko_sp_richness_Madagascar.tif")
...
head(res_complete)
write.csv(res_complete, "CHELSA_madagaskar_gecko.csv")
```
